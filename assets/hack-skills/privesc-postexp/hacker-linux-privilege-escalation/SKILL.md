---
name: hacker-linux-privilege-escalation
description: "Linux privilege escalation playbook. Use when you have low-privilege shell access and need to escalate to root via SUID/SGID binaries, capabilities, cron abuse, kernel exploits, misconfigurations, or credential harvesting on Linux systems."
metadata:
  hermes:
    tags: [privesc-postexp, "redteam", "offensive"]
    related_skills: ["hacker-linux-lateral-movement", "hacker-linux-privilege-escalation", "hacker-linux-security-bypass", "hacker-macos-security-bypass", "hacker-reverse-shell-techniques", "hacker-tunneling-and-pivoting", "hacker-windows-av-evasion", "hacker-windows-lateral-movement", "hacker-windows-privilege-escalation", "hacker-active-directory-acl-abuse"]
---
...
Also load [SUID_CAPABILITIES_TRICKS.md](references/SUID_CAPABILITIES_TRICKS.md) when you need:
...
Also load [KERNEL_EXPLOITS_CHECKLIST.md](references/KERNEL_EXPLOITS_CHECKLIST.md) when you need:
