---
name: hacker-cors-cross-origin-misconfiguration
...
metadata:
  hermes:
    ...
    related_skills: ["hacker-clickjacking", "hacker-cors-cross-origin-misconfiguration", "hacker-csp-bypass-advanced", "hacker-dangling-markup-injection", "hacker-dns-rebinding-attacks", "hacker-open-redirect", "hacker-websocket-security"]
---


# SKILL: CORS Misconfiguration — Credentialed Origins, Reflection, and Trust Boundary Errors

> **AI LOAD INSTRUCTION**: Use this skill when browsers can access authenticated APIs cross-origin. Focus on reflected origins, credentialed requests, wildcard trust, parser mistakes, and origin allowlist bypasses. For JSONP hijacking deep dives, same-origin policy internals, honeypot de-anonymization, and CORS vs JSONP comparison, load the companion [SCENARIOS.md](references/SCENARIOS.md).
