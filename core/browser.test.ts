/**
 * Chrome 集成自测：headless launch → navigate 本地目标 → CDP Network 流量捕获
 */
...
const target = createServer((q, s) => {
  if (q.url === '/') {
    ...
    s.end('<html><head><title>PentBoxTest</title></head><body><h1>hi</h1><script>fetch("/api/x")</script></body></html>')
  } else {
...
await b.launch({ headless: true, port: 9333 })

await b.navigate('http://127.0.0.1:8800/')
await new Promise((r) => setTimeout(r, 2500)) // 等页面 + fetch 完成

const title = await b.evaluate('document.title')
assert.equal(title, 'PentBoxTest', `title=${title}`)
...
assert.ok(api && api.method === 'GET', `api flow: ${JSON.stringify(api)}`)

const shot = await b.screenshot()
assert.ok(shot.length > 1000, `screenshot ${shot.length}B`)
writeFileSync('core/__shot.png', shot)

// 响应体懒取
if (api?.requestId) {
  const body = await b.getResponseBody(api.requestId)
  assert.ok(body.body.includes('"ok":true'), `body=${body.body}`)
}

console.log('web flows:')
